...
\begin{document}
	...
	\begin{enumerate}
		...
		\end{equation*}
		\item \texttt{Mathrm:} Some units and constants expressed in \texttt{$\backslash$mathrm\{$\cdot$\}} are turned into \texttt{$\backslash$name}. For example the natural exponent $\e$ can be written as \texttt{$\backslash$e}.
	\end{enumerate}

	\subsection{Equation labels in \texttt{align*} environment}

	Sometimes we want to use \texttt{align*} to avoid the unnecessary equation labels, but we want only one label in the end. In order to do that, we define a new commend \texttt{$\backslash$numberthis} and then you can tag the equation you want even in \texttt{align*}.

	\begin{example}
		The \texttt{align*} command is provided by the following code in \texttt{NotesTeX.sty}:
		\begin{verbatim}
		  \newcommand\numberthis{\addtocounter{equation}{1}\tag{\theequation}}
		\end{verbatim}
		
		And you can use it directly before \texttt{$\backslash$label} like this
		\begin{verbatim}
			\begin{align*}
				equation 1 \\
				equation 2 \numberthis \label{eq: 2}
			\end{align*}
			You can see the equation reference by using \eqref{eq: 2} commend.
		\end{verbatim}
	\end{example}
