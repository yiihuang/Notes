...
\begin{document}
	...
	\begin{theorem}
		...
		\begin{equation}
			\oint p(x) \diff x = 2 \pi \hbar (n + \mu/4) \quad \text{with } \ \mu = N_{\text{soft}} + 2N_{\text{hard}}
		\end{equation}
	...
	\subsection{Landau level for a electron in unifrom magnetic field}

	A electron is moving in the presence of a uniform magnetic field in the $z$-direction ($\bfB = B \hat{z}$).
